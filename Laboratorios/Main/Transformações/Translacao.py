...
def translate_point3D(point, tx, ty, tz, w):
    # Construção da matriz de transformação de translação
    ...
    return (translated_point_vector[0][0], translated_point_vector[1][0], translated_point_vector[2][0]) 
...
def realizar_translacao3D(cube_points_list, tx, ty, tz):
    ...
    for i in range(len(cube_points_list)):
        cube_points_list[i] = translate_point3D(cube_points_list[i], tx, ty, tz, 1)
