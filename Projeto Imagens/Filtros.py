...
def operador_prewitt(imagem):
    ...
    for y in range(1, height - 1):
        for x in range(1, width - 1):
            ...
            filtered_value_x = int(np.sum(vizinhanca_x * matriz_x))
            ...
            filtered_value_y = int(np.sum(vizinhanca_y * matriz_y))
...
def operador_roberts_cruzado(imagem):
    ...
    matriz_x = np.array([[ 1,  0],
                         [ 0, -1]])
    
    #"Operador de Roberts cruzado em Y":
    matriz_y = np.array([[ 0,  1],
                         [-1,  0]])

    for y in range(1, height - 1):
        for x in range(1, width - 1):
            # Extrair a vizinhança 2x2
            vizinhanca_x = img_array_x[y:y + 2, x:x + 2]
            vizinhanca_y = img_array_y[y:y + 2, x:x + 2]
...
def operador_roberts(imagem):
    ...
    #"Operador de Roberts em X":
    matriz_x = np.array([[ 1,  0],
                         [ -1, 0]])
    
    #"Operador de Roberts em Y":
    matriz_y = np.array([[ 1,  -1],
                         [ 0,  0]])
